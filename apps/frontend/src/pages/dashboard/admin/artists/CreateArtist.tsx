...
const BOOKING_TYPES = [
  'Local Event', 'Corporate Event', 'Wedding', 'College Fest', 'Private Party', 'International Show'
];
...
const ReviewStat = ({ label, value }: { label: string; value: string | number }) => (
  <div className="bg-white border border-surface-container rounded-2xl p-6 text-center shadow-sm">
    ...
    <p className="text-[10px] font-black text-neutral-content/30 uppercase tracking-widest mt-1">{label}</p>
  </div>
);
...
export const CreateArtist: React.FC = () => {
  ...
  const { data: artistData, isLoading: isFetchingArtist, isError, error: fetchError, refetch } = useQuery({
  ...
  useEffect(() => {
    if (isError) {
      const msg = (fetchError as any)?.response?.data?.message || 'Failed to fetch artist profile details';
      showToast('error', msg);
    }
  }, [isError, fetchError]);
  ...
  const validateStep = (step: number) => {
    ...
    if (step === 1) {
      ...
      if (!formData.shortBio || formData.shortBio.length < 10) errs.shortBio = "Bio must be at least 10 chars";
      if (!formData.longBio || formData.longBio.length < 50) errs.longBio = "Story must be at least 50 chars";
    } else if (step === 3) {
  ...
  // Tag Management
  const addTag = (field: 'premiumHighlights' | 'trustIndicators', value: string, setter: (v: string) => void) => {
  ...
  // Media Handlers
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>, type: 'profile' | 'gallery' | 'brochure') => {
    ...
    try {
      ...
      if (type === 'profile') {
        ...
        setFormData(prev => ({ ...prev, portfolioGallery: [...prev.portfolioGallery, ...res.data.urls] }));
      ...
      <div className="bg-white border border-surface-container rounded-[32px] p-8 md:p-10 mb-10 md:mb-12 shadow-sm">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-6 mb-10">
          ...
          <div className="flex flex-col items-end gap-2">
             <div className="text-[10px] font-black uppercase tracking-widest text-neutral-content/40">Onboarding Completion</div>
             <div className="flex items-center gap-4">
                ...
                <span className="text-sm font-bold text-brand-primary">{Math.round((activeStep / 5) * 100)}%</span>
             </div>
          </div>
        </div>
          ...
          {ARTIST_STEPS.map((step) => (
            ...
            >
              <div className={`w-12 h-12 rounded-2xl flex items-center justify-center text-sm font-black transition-all duration-500 border-2 ${
                activeStep === step.id ? 'bg-brand-primary border-brand-primary text-white shadow-xl shadow-brand-primary/20 scale-110' :
                activeStep > step.id ? 'bg-emerald-500 border-emerald-500 text-white' :
              ...
              <span className={`text-[10px] font-black uppercase tracking-widest hidden md:block ${
                activeStep === step.id ? 'text-brand-primary' :
                activeStep > step.id ? 'text-emerald-500' : 'text-neutral-content/30'
              }`}>
                {step.label}
              </span>
            </div>
      ...
      <div className="bg-white border border-surface-container rounded-[24px] md:rounded-[40px] shadow-sm overflow-hidden min-h-[600px]">
        ...
        <div className="p-6 md:p-10 border-b border-surface-container flex flex-col md:flex-row items-center justify-between bg-[#F8F9FA]/30 gap-4">
          <div className="space-y-1 text-center md:text-left">
            <h2 className="text-xl md:text-2xl font-bold text-neutral-content tracking-tight">Step {activeStep}: {ARTIST_STEPS[activeStep-1].label} Blueprint</h2>
          ...
          <div className="bg-[#FDF0E9] px-4 py-2 rounded-full border border-brand-primary/10 flex items-center gap-2">
             ...
             <span className="text-[9px] md:text-[10px] font-black text-brand-primary uppercase tracking-widest">Required Step</span>
          </div>
        </div>
            ...
            >
              {isFetchingArtist && isEditMode && activeStep === 1 && (
                <div className="absolute inset-0 bg-white/80 backdrop-blur-sm z-50 flex flex-col items-center justify-center gap-4">
                  ...
                  <p className="text-xs font-black uppercase tracking-widest text-neutral-content/40">Hydrating Artist Blueprint...</p>
                </div>
              )}
              ...
              {isError && isEditMode && (
                <div className="bg-rose-50 border border-rose-200 rounded-3xl p-10 text-center space-y-6">
                   ...
                   <div className="space-y-2">
                     ...
                     <p className="text-sm text-rose-700/60 font-medium max-w-sm mx-auto">The blueprint for this artist could not be retrieved from the main server.</p>
                   </div>
                   <button 
                     onClick={() => refetch()}
              ...
              {activeStep === 1 && (
                <div className="space-y-10">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-8 md:gap-y-10">
                    <InputField label="Full Legal Name" placeholder="e.g. Jonathan David Smith" value={formData.name} onChange={v => setFormData({...formData, name: v})} />
                    <InputField label="Stage/Brand Name" placeholder="e.g. J-Swift" value={formData.stageName} onChange={v => setFormData({...formData, stageName: v})} />
                    <div className="space-y-2 md:space-y-3">
                      <label className="text-[9px] md:text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Artist Category</label>
                      <select 
                    ...
                    <InputField label="Base City" placeholder="e.g. Mumbai" value={formData.city} onChange={v => setFormData({...formData, city: v})} />
                    <InputField label="State" placeholder="e.g. Maharashtra" value={formData.state} onChange={v => setFormData({...formData, state: v})} />
                    <div className="space-y-2 md:space-y-3">
                      <label className="text-[9px] md:text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Live Status</label>
                      <div className="bg-[#F8F9FA] rounded-xl md:rounded-2xl py-3.5 md:py-4 px-5 md:px-6 flex flex-col gap-4 border border-transparent">
                        <div className="flex items-center justify-between">
                           <span className="text-sm font-bold text-neutral-content/60">Visible to Public</span>
                           <button 
                             onClick={() => {
                           ...
                           >
                             <div className={`absolute top-0.5 md:top-1 left-0.5 md:left-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 ${formData.isPublished ? 'translate-x-5 md:translate-x-6' : ''}`}></div>
                           </button>
                        </div>
                        <div className="flex items-center justify-between">
                           <span className="text-sm font-bold text-neutral-content/60">Featured Artist</span>
                           <button 
                           ...
                           >
                             <div className={`absolute top-0.5 md:top-1 left-0.5 md:left-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 ${formData.isFeatured ? 'translate-x-5 md:translate-x-6' : ''}`}></div>
                           </button>
                        </div>
                        <div className="flex items-center justify-between gap-4">
                           <div className="space-y-1">
                              <span className="block text-sm font-bold text-neutral-content/60">Popular</span>
                              <span className="block text-[10px] font-black uppercase tracking-widest text-neutral-content/30">Show in Home page Popular Picks</span>
                           </div>
                           <button 
                             onClick={() => {
                               if (!formData.isPublished && !formData.showOnHome) {
                                  showToast('error', 'Only published artists can appear in Popular Picks');
                                  return;
                               }
                           ...
                           >
                             <div className={`absolute top-0.5 md:top-1 left-0.5 md:left-1 w-4 h-4 bg-white rounded-full transition-transform duration-300 ${formData.showOnHome ? 'translate-x-5 md:translate-x-6' : ''}`}></div>
                           </button>
                        </div>
                      </div>
                    </div>
              ...
              {activeStep === 2 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                   <div className="space-y-10">
                      <InputField label="Hook Line (Short Bio)" placeholder="A premium one-line punchline..." value={formData.shortBio} onChange={v => setFormData({...formData, shortBio: v})} />
                      <div className="space-y-3">
                         <label className="block text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Comprehensive Story</label>
                         <textarea
                   ...
                   <div className="space-y-10">
                      <TagArea label="Premium Highlights" placeholder="e.g. Voted Best DJ 2023" input={highlightInput} setInput={setHighlightInput} tags={formData.premiumHighlights} onAdd={() => addTag('premiumHighlights', highlightInput, setHighlightInput)} onRemove={i => removeTag('premiumHighlights', i)} />
                      <TagArea label="Trust Indicators" placeholder="e.g. Official Artist for Coca-Cola" input={trustInput} setInput={setTrustInput} tags={formData.trustIndicators} onAdd={() => addTag('trustIndicators', trustInput, setTrustInput)} onRemove={i => removeTag('trustIndicators', i)} />
                      <div className="bg-brand-secondary/5 border border-brand-secondary/10 rounded-3xl p-8 space-y-4">
                         <div className="flex items-center gap-3 text-brand-secondary">
                            <ShieldCheck className="w-5 h-5" />
                            <span className="text-[10px] font-black uppercase tracking-widest">Profile Authority</span>
                         </div>
                         <p className="text-[11px] font-medium text-neutral-content/60 leading-relaxed uppercase tracking-wide">
                            Trust indicators and highlights appear in the booking interface to maximize conversion for this talent.
                         </p>
                      </div>
                   </div>
                </div>
                ...
                <div className="space-y-16">
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    <div className="space-y-8">
                       <label className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Profile Portrait</label>
                          ...
                          >
                             {formData.profilePicture ? (
                               ...
                               <>
                                 ...
                                 <span className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest">Upload Portrait</span>
                               </>
                             )}
                    ...
                    <div className="space-y-8">
                       <label className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Performance Gallery</label>
                       <div className="bg-[#F8F9FA] border border-transparent rounded-[40px] p-8 min-h-[400px]">
                          <div className="grid grid-cols-2 gap-6">
                             {formData.portfolioGallery.map((url, i) => (
                               <div key={i} className="aspect-[4/5] bg-white rounded-2xl relative overflow-hidden group border border-surface-container shadow-sm">
                                  <img src={resolveMediaUrl(url)} alt="Gallery" className="w-full h-full object-cover" />
                                  <button onClick={() => setFormData(p => ({ ...p, portfolioGallery: p.portfolioGallery.filter(u => u !== url) }))} className="absolute top-2 right-2 p-2 bg-white rounded-lg text-error opacity-0 group-hover:opacity-100 transition-all">
                             ...
                             >
                                {uploading.gallery ? <Loader2 className="w-6 h-6 animate-spin" /> : <Plus className="w-8 h-8" />}
                                <span className="text-[9px] font-black uppercase tracking-widest">Add Media</span>
                                <input type="file" ref={galleryInputRef} className="hidden" multiple accept="image/*" onChange={e => handleFileUpload(e, 'gallery')} />
                             </div>
                          </div>
                       </div>
                    </div>
                  ...
                  <div className="grid grid-cols-1 lg:grid-cols-2 gap-16">
                    <div className="space-y-8">
                       <label className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Video Showreel Links</label>
                       <div className="bg-white border border-surface-container rounded-[32px] p-8 space-y-6">
                          <div className="flex gap-4">
                             <div className="flex-1 relative">
                                ...
                                <input type="text" placeholder="YouTube/Vimeo URL..." value={videoLinkInput} onChange={e => setVideoLinkInput(e.target.value)} className="w-full bg-[#F8F9FA] border border-transparent rounded-2xl py-3.5 pl-12 pr-6 text-sm font-bold text-neutral-content focus:outline-none focus:border-brand-secondary transition-all" />
                             </div>
                             <button onClick={() => { if(videoLinkInput) { setFormData(p => ({ ...p, youtubeLinks: [...p.youtubeLinks, videoLinkInput] })); setVideoLinkInput(''); } }} className="bg-brand-secondary text-white px-6 rounded-2xl font-black text-[10px] uppercase tracking-widest">Add</button>
                          </div>
                          <div className="space-y-3">
                             {formData.youtubeLinks.map((link, i) => (
                               <div key={i} className="flex items-center justify-between p-4 bg-[#F8F9FA] rounded-xl text-xs font-bold text-neutral-content/60">
                                  <div className="flex items-center gap-3"><Youtube className="w-4 h-4 text-rose-500" /><span className="truncate max-w-[200px]">{link}</span></div>
                                  <button onClick={() => setFormData(p => ({ ...p, youtubeLinks: p.youtubeLinks.filter(l => l !== link) }))}><X className="w-4 h-4" /></button>
                               </div>
                             ))}
                          </div>
                       </div>
                    ...
                    <div className="space-y-8">
                       <label className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Artist Brochure (PDF)</label>
                       <div 
                          ...
                          {formData.brochure ? (
                            <div className="flex items-center gap-4 text-brand-success">
                               ...
                               <div className="text-left">
                                  <p className="text-sm font-bold text-neutral-content">Brochure Uploaded</p>
                                  <p className="text-[10px] font-black uppercase tracking-widest">Click to Replace</p>
                               </div>
                            </div>
                          ) : uploading.brochure ? (
                            ...
                            <>
                               ...
                               <span className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest">Upload Profile Deck</span>
                            </>
                          )}
                          <input type="file" ref={brochureInputRef} className="hidden" accept=".pdf" onChange={e => handleFileUpload(e, 'brochure')} />
                       </div>
                    </div>
                  </div>
                </div>
              ...
              {activeStep === 4 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-x-16 gap-y-12">
                   <div className="space-y-10">
                      <InputField label="Starting Performance Fee (₹)" placeholder="e.g. 50,000" type="number" value={formData.startingPrice} onChange={v => setFormData({...formData, startingPrice: v})} />
                      <div className="grid grid-cols-2 gap-6">
                         <InputField label="Price Floor (₹)" placeholder="Min" type="number" value={formData.priceRange.min} onChange={v => setFormData({...formData, priceRange: { ...formData.priceRange, min: v }})} />
                         <InputField label="Price Ceiling (₹)" placeholder="Max" type="number" value={formData.priceRange.max} onChange={v => setFormData({...formData, priceRange: { ...formData.priceRange, max: v }})} />
                      </div>
                      <div className="space-y-4">
                         <label className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Booking Capability</label>
                         <div className="grid grid-cols-2 gap-3">
                            {BOOKING_TYPES.map(type => (
                              <button key={type} onClick={() => setFormData(p => ({ ...p, bookingTypes: p.bookingTypes.includes(type) ? p.bookingTypes.filter(t => t !== type) : [...p.bookingTypes, type] }))} className={`p-4 rounded-2xl text-[10px] font-black uppercase tracking-widest text-left border-2 transition-all ${formData.bookingTypes.includes(type) ? 'bg-brand-primary border-brand-primary text-white shadow-lg shadow-brand-primary/20' : 'bg-[#F8F9FA] border-transparent text-neutral-content/60'}`}>{type}</button>
                            ))}
                         </div>
                      </div>
                   </div>
                   <div className="space-y-10">
                      <div className="space-y-3">
                         <label className="block text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Pricing & Logistics Notes</label>
                         <textarea placeholder="Travel, accommodation, and rider requirements..." value={formData.pricingNotes} onChange={e => setFormData({ ...formData, pricingNotes: e.target.value })} className="w-full h-44 bg-[#F8F9FA] border border-transparent rounded-[24px] px-6 py-5 text-sm font-semibold text-neutral-content focus:outline-none focus:border-brand-primary transition-all resize-none shadow-sm" />
                      </div>
                      <div className="space-y-3">
                         <label className="block text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">Market Position (Tier)</label>
                         <select className="w-full bg-[#F8F9FA] border border-transparent rounded-2xl py-4 px-6 text-sm font-bold text-neutral-content appearance-none cursor-pointer focus:outline-none focus:border-brand-primary transition-all" value={formData.premiumTier} onChange={e => setFormData({ ...formData, premiumTier: e.target.value })}>
                ...
                <div className="space-y-10">
                   <div className="bg-brand-success/5 border border-brand-success/10 rounded-[32px] p-10 flex flex-col md:flex-row items-center gap-8">
                      <div className="w-24 h-24 bg-white rounded-3xl overflow-hidden border-2 border-brand-success/20 shadow-sm shrink-0">
                         <img src={resolveMediaUrl(formData.profilePicture)} alt="Final Preview" className="w-full h-full object-cover" />
                      </div>
                      <div className="flex-1 space-y-2 text-center md:text-left">
                         <h3 className="text-3xl font-bold text-neutral-content tracking-tight">{formData.stageName}</h3>
                         <p className="text-neutral-content/40 font-medium">{formData.city}, {formData.state} • {formData.categoryId ? categories.find(c => c._id === formData.categoryId)?.name : 'Uncategorized'}</p>
                         <div className="flex flex-wrap justify-center md:justify-start gap-2 pt-2">
                            <span className="bg-brand-primary text-white px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">{formData.premiumTier}</span>
                            <span className="bg-brand-secondary text-white px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest">{formData.isPublished ? 'Live' : 'Draft'}</span>
                         </div>
                      </div>
                      <div className="text-center md:text-right">
                         <p className="text-2xl font-bold text-neutral-content">₹ {Number(formData.startingPrice).toLocaleString()}</p>
                         <p className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest mt-1">Starting Performance Fee</p>
                      </div>
                   </div>
                   ...
                   <div className="p-10 bg-[#F8F9FA] rounded-[32px] space-y-4">
                      <h4 className="text-[10px] font-black text-neutral-content/40 uppercase tracking-widest">Administrative Bio Summary</h4>
                      <p className="text-sm font-medium text-neutral-content/60 leading-relaxed">{formData.shortBio || "No hook line provided."}</p>
                   </div>
                </div>
              )}
            </motion.div>
        ...
        <div className="p-6 md:p-10 border-t border-surface-container flex flex-col md:flex-row items-center justify-between bg-[#F8F9FA]/30 gap-6">
          ...
          <div className="w-full md:w-auto flex items-center justify-between md:justify-end gap-6 md:gap-8">
            ...
            >
              {activeStep === 5 ? (isLoading ? <Loader2 className="w-5 h-5 animate-spin" /> : 'Finalize & Sync') : <>Next Step <ChevronRight className="w-4 h-4 md:w-5 md:h-5" /></>}
            </button>
          </div>
        </div>
      </div>
...
const InputField = ({ label, placeholder, value, onChange, type = "text" }: InputFieldProps) => (
  <div className="space-y-2 md:space-y-3">
    <label className="text-[9px] md:text-[10px] font-black text-neutral-content/40 uppercase tracking-widest ml-1">{label}</label>
    <input 
      type={type} 
  ...
  <div className="space-y-4">
    ...
    <div className="bg-[#F8F9FA] border border-transparent rounded-xl md:rounded-2xl p-4 flex flex-wrap items-center gap-3 min-h-[64px]">
      {tags.map((tag: string, i: number) => (
        <div key={i} className="bg-brand-secondary text-white px-3 md:px-4 py-1.5 md:py-2 rounded-lg md:rounded-xl text-[9px] md:text-[10px] font-black uppercase tracking-widest flex items-center gap-2">
          {tag} <button onClick={() => onRemove(i)}><X className="w-2.5 h-2.5 md:w-3 md:h-3" /></button>
        </div>
      ))}
      <input 
        type="text" 
