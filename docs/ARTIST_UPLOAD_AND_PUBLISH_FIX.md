...
Media files must be uploaded _before_ the artist is provisioned. The backend exposes two dedicated, deterministically-routed endpoints:
...
_Note: The backend now serves these images statically via `/uploads/artists/_`.\*
...
Upload routes have been strictly ordered in `admin.routes.ts` _before_ the `/:id` parameterized routes. This guarantees that `upload-profile` is not interpreted as an artist ID by Express.
