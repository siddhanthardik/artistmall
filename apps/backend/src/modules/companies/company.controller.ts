...
export const updateManagementProfile = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const profile = await CompanyService.createOrUpdateManagementProfile(
      req.user!.userId,
      req.body,
    );
...
export const submitManagementKyc = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const profile = await CompanyService.submitManagementKyc(
      req.user!.userId,
      documentUrl,
      gstNumber,
    );
...
export const submitBookingKyc = async (req: Request, res: Response, next: NextFunction) => {
  try {
    ...
    const profile = await CompanyService.submitBookingKyc(
      req.user!.userId,
      industryType,
      billingAddress,
    );
