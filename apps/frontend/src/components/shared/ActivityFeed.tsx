...
const DUMMY_FEED: FeedItem[] = [
  ...
  },
  {
    ...
    description: 'Reliance Events countered with ₹8.5L for Neha Kakkar.',
    timeAgo: '5h ago',
  },
  {
    id: '3',
    ...
    description: 'Your KYC documents are still pending upload.',
    timeAgo: '1d ago',
  },
  {
    id: '4',
    ...
    description: 'Sunburn Fest requested a quote for DJ Chetas.',
    timeAgo: '2d ago',
  },
];
...
export const ActivityFeed: React.FC = () => {
  const getIcon = (type: FeedItem['type']) => {
    switch (type) {
      ...
      case 'ACTION':
        return <Plus className="w-4 h-4 text-gold-400" />;
    }
  };
        ...
        <div className="relative border-l border-slate-700 ml-3 space-y-6">
          {DUMMY_FEED.map((item) => (
            <div key={item.id} className="relative pl-6">
              {/* Timeline dot */}
              <div
                className={`absolute -left-3.5 top-1 w-7 h-7 rounded-full border ${getBgColor(item.type)} flex items-center justify-center`}
              >
                {getIcon(item.type)}
              </div>
