      ...
      <div className="flex items-end justify-between">
        <div>
          ...
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em] mt-2">
            GMV Tracking & Platform Commission Settlement
          </p>
        </div>
            ...
            <tbody className="divide-y divide-white/5">
              {DUMMY_COMMISSIONS.map((comm) => (
                <tr key={comm.id} className="hover:bg-white/[0.02] transition-colors group">
                  ...
                  <td className="px-6 py-6 text-sm font-black text-white uppercase tracking-tight">
                    {comm.agency}
                  </td>
                  <td className="px-6 py-6">
        ...
        <div className="flex items-center gap-6">
          ...
          <div>
            <h4 className="text-white font-black uppercase text-xs tracking-widest">
              Financial Integrity Shield
            </h4>
            <p className="text-slate-500 text-[10px] font-bold uppercase tracking-widest mt-1">
              All transactions are audit-logged and immutable once settled.
            </p>
          </div>
        </div>
...
const FinanceCard = ({ label, value, sub, color, icon: Icon, alert }: any) => {
  ...
  return (
    <div className="bg-[#111] border border-white/5 rounded-[40px] p-10 shadow-2xl relative group overflow-hidden">
      {alert && (
        <div className="absolute top-6 right-6 w-2 h-2 bg-orange-500 rounded-full animate-ping"></div>
      )}
      <div
        className={`w-14 h-14 rounded-2xl flex items-center justify-center mb-8 ${colors[color]} border transition-transform group-hover:scale-110`}
      >
