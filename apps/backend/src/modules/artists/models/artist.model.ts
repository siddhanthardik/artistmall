...
export type AvailabilityStatus = 'AVAILABLE' | 'TENTATIVE' | 'BLOCKED' | 'TOURING' | 'SEASONAL_HOLD';
export type VerificationStatus = 'DRAFT' | 'PENDING_REVIEW' | 'APPROVED' | 'REJECTED' | 'NEEDS_UPDATE' | 'PUBLISHED';
...
artistSchema.pre('save', async function (next) {
  ...
  if (self.isNew || self.isModified('stageName') || self.isModified('name')) {
    ...
    while (true) {
      const existing = await model.findOne({ slug: newSlug, _id: { $ne: self._id } }).select('_id').lean();
      if (!existing) break;
