...
export const ModerationCenter: React.FC = () => {
  ...
  const toggleFeaturedMutation = useMutation({
    mutationFn: ({ id, isFeatured, premiumTier }: { id: string, isFeatured: boolean, premiumTier: string }) => 
  ...
  return (
    <div className="space-y-10">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter uppercase">Monetization & Visibility</h1>
          <p className="text-slate-500 font-bold text-[10px] uppercase tracking-[0.3em] mt-1">Premium Ranking & Featured Inventory Management</p>
        </div>
        <div className="flex items-center gap-4 bg-white border border-slate-200 rounded-2xl px-6 py-3 shadow-sm">
           <Zap className="w-4 h-4 text-brand-orange" />
           <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">Global Ranking Priority Active</span>
        </div>
      </div>
        ...
        <table className="w-full text-left">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-200">
              <th className="px-10 py-6 text-[10px] font-black text-slate-500 uppercase tracking-widest">Talent Profile</th>
              <th className="px-6 py-6 text-[10px] font-black text-slate-500 uppercase tracking-widest">System Status</th>
              <th className="px-6 py-6 text-[10px] font-black text-slate-500 uppercase tracking-widest">Marketplace Visibility</th>
              <th className="px-10 py-6 text-[10px] font-black text-slate-500 uppercase tracking-widest text-right">Monetization Controls</th>
            </tr>
          </thead>
          <tbody className="divide-y divide-slate-100">
            {isLoading ? (
              <tr>
                <td colSpan={4} className="py-20 text-center"><Loader2 className="w-8 h-8 animate-spin mx-auto text-brand-orange" /></td>
              </tr>
            ) : artists.length === 0 ? (
              <tr><td colSpan={4} className="py-20 text-center text-slate-400 font-bold uppercase text-xs tracking-widest">No talent records found</td></tr>
            ) : (
                ...
                <tr key={artist._id} className="hover:bg-slate-50/50 transition-colors group">
                  <td className="px-10 py-6">
                    <div className="flex items-center gap-5">
                       ...
                       <div>
                          <p className="text-sm font-black text-slate-900 uppercase tracking-tight">{artist.stageName}</p>
                          <p className="text-[9px] font-bold text-slate-400 uppercase tracking-widest mt-0.5">{artist.categoryId?.name}</p>
                       </div>
                    </div>
                  </td>
                  <td className="px-6 py-6">
                     <span className={`px-3 py-1 rounded-full text-[9px] font-black uppercase tracking-widest border ${
                       artist.verificationStatus === 'APPROVED' ? 'bg-green-50 text-brand-success border-green-100' : 
                       artist.verificationStatus === 'PENDING_REVIEW' ? 'bg-orange-50 text-brand-orange border-orange-100' :
                     ...
                     <div className="flex items-center gap-3">
                        {artist.isFeatured ? (
                          <div className="flex items-center gap-2 text-brand-orange">
                             ...
                             <span className="text-[10px] font-black uppercase tracking-widest">FEATURED: {artist.premiumTier}</span>
                          </div>
                        ) : (
                          <span className="text-[10px] font-bold text-slate-400 uppercase tracking-widest">ORGANIC RANKING</span>
                        )}
                     </div>
                  </td>
                  <td className="px-10 py-6 text-right">
                     <div className="flex items-center justify-end gap-3">
                        <button 
                          onClick={() => toggleFeaturedMutation.mutate({ id: artist._id, isFeatured: false, premiumTier: 'STANDARD' })}
                        ...
                        <button 
                          onClick={() => toggleFeaturedMutation.mutate({ id: artist._id, isFeatured: true, premiumTier: 'SILVER' })}
                        ...
                        <button 
                          onClick={() => toggleFeaturedMutation.mutate({ id: artist._id, isFeatured: true, premiumTier: 'GOLD' })}
