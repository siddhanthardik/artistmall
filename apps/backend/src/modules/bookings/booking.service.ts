...
export class BookingService {
  ...
  static async createDraft(bookingCompanyId: string, data: any) {
    const artist = await ArtistModel.findOne({ _id: data.artistId, status: 'APPROVED', isDeleted: false });
  ...
  static async submitRequest(userId: string, bookingCompanyId: string, bookingId: string) {
    const booking = await BookingRequestModel.findOne({ _id: bookingId, bookingCompanyId, status: 'DRAFT' });
  ...
  static async negotiate(userId: string, bookingId: string, message: string, proposedPrice: number, isCounterOffer: boolean) {
    ...
    try {
      ...
      await this.logStatusChange(booking._id, oldStatus, 'NEGOTIATING', userId, 'Counter offer submitted', session);
  ...
  static async payAdvance(userId: string, bookingId: string) {
    ...
    const booking = await BookingRequestModel.findOne({ _id: bookingId, status: 'ADVANCE_PENDING' });
    ...
    await this.logStatusChange(booking._id, oldStatus, 'CONFIRMED', userId, 'Advance payment received');
    return booking;
  }

  static async completeBooking(userId: string, bookingId: string) {
    const booking = await BookingRequestModel.findOne({ _id: bookingId, status: 'CONFIRMED' }).populate('artistId');
    ...
    try {
      ...
      await this.logStatusChange(booking._id, oldStatus, 'COMPLETED', userId, 'Event completed', session);
  ...
  static async adminOverrideStatus(adminId: string, bookingId: string, newStatus: string, reason: string) {
    const booking = await BookingRequestModel.findById(bookingId);
    if (!booking) throw new AppError('Booking not found', 404);
    ...
    try {
      ...
      await this.logStatusChange(booking._id, oldStatus, newStatus, adminId, `ADMIN OVERRIDE: ${reason}`, session);
