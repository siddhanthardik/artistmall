...
export const AdminLayout: React.FC = () => {
  ...
  const navItems = [
    ...
    { label: 'Artists', icon: Users, to: '/admin/artists', exact: true, show: can('artist.view', ['SUPER_ADMIN', 'SUB_ADMIN', 'INTERNAL_OPS']) },
    { label: 'Add Artist', icon: UserPlus, to: '/admin/artists/create', exact: true, show: can('artist.create', ['SUPER_ADMIN', 'SUB_ADMIN']) },
    { label: 'Categories', icon: Grid, to: '/admin/categories', show: can('settings.manage', ['SUPER_ADMIN', 'SUB_ADMIN']) },
    { label: 'Clients', icon: Briefcase, to: '/admin/clients', show: can('settings.manage', ['SUPER_ADMIN', 'SUB_ADMIN']) },
    { label: 'Featured Artists', icon: Star, to: '/admin/moderation', show: can('artist.edit', ['SUPER_ADMIN', 'SUB_ADMIN']) },
    { label: 'Hero Banners', icon: ImageIcon, to: '/admin/hero-banners', show: can('settings.manage', ['SUPER_ADMIN', 'SUB_ADMIN']) },
    { label: 'Leads / Enquiries', icon: MessageCircle, to: '/admin/leads', show: can('lead.view', ['SUPER_ADMIN', 'SUB_ADMIN', 'INTERNAL_OPS']) },
    { label: 'Settings', icon: Settings, to: '/admin/settings', show: can('settings.manage', ['SUPER_ADMIN']) },
  ];
            ...
            >
              <div className="p-8 flex items-center justify-between">
                 <div className="space-y-1">
                   <h1 className="text-xl font-bold text-neutral-content tracking-tighter">Artist Mall <span className="text-brand-primary">Admin</span></h1>
                   <p className="text-[10px] font-black text-neutral-content/40 uppercase tracking-[0.2em]">Management Portal</p>
                 </div>
                 <button onClick={() => setIsMobileMenuOpen(false)} className="p-2 text-neutral-content/40">
                   <X className="w-6 h-6" />
                 </button>
              </div>
                  ...
                  return (
                    <Link 
                      ...
                      className={`flex items-center gap-4 px-6 py-4 rounded-2xl transition-all ${
                        isActive ? 'bg-[#FDF0E9] text-brand-primary font-bold' : 'text-neutral-content/60 font-medium'
                      }`}
                    >
              ...
              <div className="p-6 border-t border-surface-container">
                 <div className="flex items-center gap-4">
                    <div className="w-10 h-10 rounded-xl bg-surface-container overflow-hidden">
                       <img src="https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&q=80&w=100" alt="Avatar" className="w-full h-full object-cover" />
                    </div>
                    <div className="flex-1 min-w-0">
                       <p className="text-sm font-bold text-neutral-content truncate">{user?.fullName || 'Admin User'}</p>
                       <p className="text-[10px] font-bold text-neutral-content/40 uppercase tracking-widest mt-0.5">{user?.roleName || user?.role}</p>
                    </div>
                    <button onClick={handleLogout} className="p-2 text-neutral-content/20 hover:text-error transition-colors">
      ...
      >
        <div className="p-8 mb-6 overflow-hidden flex items-center justify-between">
          {isSidebarOpen ? (
            ...
            <div className="w-8 h-8 bg-brand-primary rounded-lg flex items-center justify-center font-bold text-white text-xs mx-auto">AM</div>
          )}
        </div>
        ...
        <nav className="flex-1 px-4 space-y-1">
          {filteredNav.map((item: any) => {
            ...
            return (
              ...
              >
                <item.icon className={`w-5 h-5 shrink-0 ${isActive ? 'text-brand-primary' : 'text-neutral-content/40 group-hover:text-neutral-content'}`} />
                {isSidebarOpen && (
                  <motion.span initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} className="text-sm">
                    {item.label}
                  </motion.span>
                )}
        ...
        <header className="h-20 bg-white border-b border-surface-container px-6 md:px-10 flex items-center justify-between sticky top-0 z-40">
          <div className="flex items-center gap-4">
            ...
            <div className="md:hidden">
               <h1 className="text-sm font-bold text-neutral-content tracking-tighter">Artist Mall <span className="text-brand-primary">Admin</span></h1>
            </div>
          </div>
