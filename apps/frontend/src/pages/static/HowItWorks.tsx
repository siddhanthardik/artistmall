      ...
      <section className="relative pt-40 pb-20 bg-white border-b border-slate-100">
        <div className="max-w-7xl mx-auto px-6 lg:px-10">
          ...
          <p className="text-slate-500 text-xl md:text-2xl font-medium tracking-tight max-w-2xl leading-relaxed">
            See how artist discovery, inquiry handling, and event coordination work on The Artist
            Mall.
          </p>
        </div>
      </section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-20 items-center">
            ...
            <div className="bg-white border border-slate-200 rounded-[40px] p-12 shadow-2xl relative overflow-hidden group">
              ...
              <p className="text-slate-500 font-medium leading-relaxed mb-10">
                We review artist profiles before publishing, including identity information, profile
                media, category details, and technical requirements where available.
              </p>
...
const TimelineStep = ({ number, icon: Icon, title, description, side, color }: any) => {
  ...
  return (
    <div
      className={`flex flex-col lg:flex-row items-center gap-12 lg:gap-0 relative ${isLeft ? 'lg:flex-row' : 'lg:flex-row-reverse'}`}
    >
      {/* Content Side */}
      <div
        className={`flex-1 w-full ${isLeft ? 'lg:text-right lg:pr-24' : 'lg:text-left lg:pl-24'}`}
      >
        <motion.div
        ...
        >
          <div
            className={`inline-flex items-center gap-4 text-xs font-black uppercase tracking-widest ${isLeft ? 'flex-row' : 'flex-row-reverse'}`}
          >
            <span
              className={`w-8 h-8 rounded-lg flex items-center justify-center border font-black ${colorMap[color]}`}
            >
              {number}
            </span>
      ...
      <div className="flex-1 w-full">
        ...
        >
          <div className="w-full h-full flex items-center justify-center">
            <Icon
              className={`w-24 h-24 ${isLeft ? 'text-brand-orange' : 'text-brand-blue'} opacity-20`}
            />
          </div>
        </motion.div>
      </div>
