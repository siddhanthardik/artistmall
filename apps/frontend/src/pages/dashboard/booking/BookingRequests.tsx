            ...
            <tbody className="divide-y divide-slate-100">
              ...
              {requests?.map((req: Booking) => (
                <tr key={req._id} className="hover:bg-slate-50/50 transition-all group">
                  ...
                  <td className="px-8 py-5">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="font-black text-slate-900">
                        {req.artistId?.name || 'Unknown Artist'}
                      </span>
                    </div>
                  </td>
                  <td className="px-8 py-5 font-bold text-slate-600">
                    {new Date(req.eventDate).toLocaleDateString(undefined, {
                      month: 'short',
                      day: 'numeric',
                      year: 'numeric',
                    })}
                  ...
                  <td className="px-8 py-5">
                    <span className="font-black text-slate-900">
                      ₹{req.currentOffer?.toLocaleString()}
                    </span>
                  </td>
                  <td className="px-8 py-5">
                    <StatusBadge status={req.status as StatusType} />
                  </td>
                  ...
                  <td className="px-8 py-5 text-right">
                    {req.status === 'NEGOTIATING' ? (
                      <Link
                        to={`/dashboard/booking/negotiations?id=${req._id}`}
                        className="bg-[#1E4DB7] hover:bg-[#163B8C] text-white px-4 py-2 rounded-lg text-xs font-black shadow-md shadow-blue-500/10 transition-all inline-flex items-center gap-2"
                      >
