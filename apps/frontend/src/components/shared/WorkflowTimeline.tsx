...
export type BookingStage = 'REQUESTED' | 'NEGOTIATING' | 'ADVANCE_PENDING' | 'CONFIRMED' | 'COMPLETED' | 'CANCELLED';
    ...
    <div className="w-full py-4">
      <div className="flex items-center justify-between relative px-2">
        ...
        {STAGES.map((stage, index) => {
          ...
          let nodeClasses = "w-10 h-10 rounded-full border-2 flex items-center justify-center z-10 transition-all bg-white ";
          
          if (isCompleted) {
            nodeClasses += "border-[#1E4DB7] text-[#1E4DB7] bg-blue-50";
          } else if (isActive) {
            nodeClasses += "border-[#FF7A2F] text-[#FF7A2F] shadow-lg shadow-orange-500/20 scale-110";
          } else {
          ...
          return (
            <div key={stage.id} className="flex flex-col items-center gap-2 relative z-10">
              <div className={nodeClasses}>
                {isCompleted ? <Check className="w-5 h-5 stroke-[3px]" /> : <span className="text-sm font-black">{index + 1}</span>}
              </div>
              <span className={`text-[10px] font-black uppercase tracking-widest absolute top-12 whitespace-nowrap transition-colors ${isActive ? 'text-[#FF7A2F]' : isCompleted ? 'text-slate-900' : 'text-slate-300'}`}>
                {stage.label}
              </span>
            </div>
          );
